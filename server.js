...
const { col, col2, col3 } = require('./config');
const app = express();
...
app.get('/story/:lang', async (req, res) => {
    try {
        ...
        const stories = snapshot.docs.map(doc => {
            ...
            const filteredData = {
                id: doc.id,
                question: data.question[lang] || data.question['ar'], // fallback to English
                answer: data.answer[lang] || data.answer['ar']      // fallback to English
            };
            return filteredData;
        }).filter(item => item.question && item.answer); // filter out items with missing translations
